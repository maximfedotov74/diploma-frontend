...
export const nextMiddlewareCheckAdmin = async (request: NextRequest) => {
	const API_URl = process.env.API_URL as string;
	...
	notFound.pathname = NOT_FOUND_ROUTE;
	const newNextResponse = NextResponse.next();
	try {
		const rolesResponse = await fetch(`${API_URl}/api/role/check-for-admin`, {
			method: 'GET',
			headers: {
				Authorization: `Bearer ${accessToken}`,
				'User-Agent': userAgent,
		...
		if (!rolesResponse.ok) {
			if (rolesResponse.status === 401) {
				...
				if (refreshResponse.ok) {
					const refreshData =
						(await refreshResponse.json()) as ModelLoginResponse;
					const newRolesResponse = await fetch(
						`${API_URl}/api/role/check-for-admin`,
						{
							method: 'GET',
							headers: {
								Authorization: `Bearer ${refreshData.tokens.access_token}`,
